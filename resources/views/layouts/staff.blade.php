...
<head>
    ...
    <title>Nhân Viên - {{ config('app.name', 'Gym Fit') }}</title>
    ...
    <div class="flex h-screen overflow-hidden" x-data="{ sidebarOpen: true }">
        ...
        <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 transition-transform duration-300 transform lg:translate-x-0 lg:static lg:inset-0"
            ...
            <nav class="mt-6 px-4 space-y-1.5 overflow-y-auto max-h-[calc(100vh-160px)] no-scrollbar">
                <a href="{{ route('staff.dashboard') }}" class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 text-slate-300 {{ request()->routeIs('staff.dashboard') ? 'active' : '' }}">
                ...
                <div class="pt-5 pb-2 px-4 text-xs font-semibold text-slate-500 uppercase tracking-widest leading-loose whitespace-nowrap">Dịch vụ & Check-in</div>

                <a href="{{ route('staff.checkin.station') }}" class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 text-slate-300 {{ request()->routeIs('staff.checkin.station') ? 'active' : '' }}">
                    <i class="fa-solid fa-qrcode w-5"></i>
                    <span class="font-medium text-[15px]">Trạm Điểm Danh QR</span>
                </a>
                ...
                <div class="flex items-center gap-5">
                    ...
                    <div class="flex items-center gap-3 cursor-pointer group">
                        <div class="text-right hidden lg:block">
                            ...
                            <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ Auth::user()->role === 'admin' ? 'Quản trị viên' : (Auth::user()->role === 'trainer' ? 'Huấn luyện viên' : 'Nhân viên') }}</div>
                        </div>
                        <div class="relative">
