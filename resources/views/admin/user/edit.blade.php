...
<div class="max-w-5xl mx-auto pb-12">
    <div class="bg-white rounded-[32px] shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="px-8 py-8 border-b border-gray-50 bg-slate-50/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-black text-slate-900 tracking-tighter uppercase">Chỉnh sửa tài khoản</h2>
                <p class="text-sm text-slate-400 font-bold tracking-tighter uppercase italic">Cập nhật thông tin cho tài khoản: #{{ $userDetail->id }} — {{ $userDetail->name }}</p>
            </div>
        ...
        <form action="{{ route('admin.users.update', $userDetail->id) }}" method="POST" enctype="multipart/form-data" class="p-8 md:p-12">
            @csrf
            @method('PUT')
            
            <div class="flex flex-col lg:flex-row gap-12">
                <!-- Left Column: Avatar Update & Preview -->
                <div class="w-full lg:w-1/3 flex flex-col items-center">
                    <div class="space-y-6 text-center w-full">
                        <label class="block text-xs font-black text-slate-400 uppercase tracking-[.2em] mb-4">Ảnh đại diện hiện tại</label>
                        
                        <div class="relative group mx-auto">
                            <div id="avatarPreview" class="w-48 h-48 rounded-[40px] bg-slate-100 border-2 border-slate-200 flex items-center justify-center overflow-hidden transition-all duration-300 group-hover:border-blue-300 group-hover:shadow-xl shadow-blue-600/5 {{ $userDetail->avatar_url ? 'border-blue-500 ring-4 ring-blue-500/10' : '' }}">
                                <i class="fa-solid fa-user-astronaut text-5xl text-slate-300 group-hover:scale-110 transition-transform duration-500 {{ $userDetail->avatar_url ? 'hidden' : '' }}"></i>
                                <img id="previewImg" src="{{ $userDetail->avatar_url }}" class="w-full h-full object-cover {{ $userDetail->avatar_url ? '' : 'hidden' }}" alt="Avatar Preview">
                            </div>
                            
                            <!-- Status Badge -->
                            <div id="checkBadge" class="absolute -bottom-2 -right-2 w-10 h-10 bg-emerald-500 text-white rounded-2xl flex items-center justify-center shadow-lg transform {{ $userDetail->avatar_url ? 'scale-100' : 'scale-0' }} transition-transform duration-300 border-4 border-white">
                            ...
                            <label for="avatarInput" class="absolute inset-0 cursor-pointer flex items-center justify-center bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity rounded-[40px]">
                                <span class="bg-white/20 backdrop-blur-md px-4 py-2 rounded-xl text-white text-[10px] font-black uppercase tracking-widest border border-white/30">Thay đổi ảnh</span>
                            </label>
                        </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <div id="fileName" class="text-[10px] text-slate-400 font-bold uppercase tracking-tight italic">
                                {{ $userDetail->avatar_url ? 'Đã có ảnh đại diện' : 'Chưa có tệp nào được chọn' }}
                            </div>
                ...
                <div class="flex-1 space-y-8">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-3">
                            ...
                            <input type="text" name="name" 
                                ...
                                placeholder="Nguyễn Văn A" value="{{ old('name', $userDetail->name) }}">
                        ...
                        <div class="space-y-3">
                            ...
                            <input type="email" name="email" 
                                class="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all outline-none"
                                placeholder="name@example.com" value="{{ old('email', $userDetail->email) }}">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="relative group">
                                @if($userDetail->role === 'admin')
                                    ...
                                    <div class="w-full px-6 py-4 bg-red-50 border border-red-100 rounded-2xl text-sm font-black text-red-600 cursor-not-allowed">
                        ...
                        <!-- Password -->
                        <div class="space-y-3">
                            <label class="block text-xs font-black text-slate-900 uppercase tracking-[.2em] ml-1">Mật khẩu (Để trống nếu không đổi)</label>
                            <div class="relative">
                    ...
                    <div class="pt-10 flex flex-col sm:flex-row items-center justify-end gap-4">
                        <a href="{{ route('admin.users.index') }}" class="w-full sm:w-auto px-10 py-4 bg-slate-100 text-slate-500 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-200 active:scale-95 transition-all text-center">
                            Hủy bỏ
                        </a>
                        <button type="submit" class="w-full sm:w-auto px-12 py-4 bg-blue-600 text-white rounded-2xl font-black text-xs uppercase tracking-[.2em] shadow-xl shadow-blue-600/30 hover:bg-blue-700 hover:-translate-y-1 active:scale-95 transition-all">
                            Lưu thay đổi <i class="fa-solid fa-save ml-2"></i>
                        </button>
                    </div>
                </div>
            </div>
...
<script>
    ...
    avatarInput.addEventListener('change', function(e) {
        ...
        if (file) {
            ...
            fileNameDisplay.textContent = '{{ $userDetail->avatar_url ? "Đã có ảnh đại diện" : "Chưa có tệp nào được chọn" }}';
